            ...
            if (species != SPECIES_NONE)
            {
                if (species == SPECIES_ZOMBIE_FAIRY && GetMonData(mon, MON_DATA_LANGUAGE) != LANGUAGE_JAPANESE)
                {
                    ...
                    if (!StringCompareWithoutExtCtrlCodes(name, sText_ShedinjaJP))
                        SetMonData(mon, MON_DATA_NICKNAME, gSpeciesNames[SPECIES_ZOMBIE_FAIRY]);
                }
            }
        }
        return TRUE;
...
static u8 CheckValidityOfTradeMons(u8 *aliveMons, u8 playerPartyCount, u8 cursorPos)
{
    ...
    if ((partnerSpecies == SPECIES_AYAKASHI || partnerSpecies == SPECIES_SATSUKI)
...
static u32 CanTradeSelectedMon(struct Pokemon * playerParty, int partyCount, int monIdx)
{
    ...
    if (species[monIdx] == SPECIES_AYAKASHI || species[monIdx] == SPECIES_SATSUKI)
    {
...
{
    if (species == SPECIES_AYAKASHI || species == SPECIES_SATSUKI)
    {

...
static void InitResetRtcScreenBgAndWindow(void);
...
static const struct ResetRtcStruct sInputMap[] =
{
    [SELECTION_DAYS - 1] = {
    ...
    if (state != sprite->sState)
    {
        ...
        switch (state)
        {
            ...
            case SELECTION_SECS:
                ...
                break;
            case SELECTION_CONFIRM:
                // This one uses the up arrow above
...
static void Task_ResetRtc_HandleInput(u8 taskId)
{
    ...
    const struct ResetRtcStruct *selectionInfo = &sInputMap[selection - 1];
...
static void InitResetRtcScreenBgAndWindow(void)
{
    ...
    ResetBgsAndClearDma3BusyFlags(FALSE);
    InitBgsFromTemplates(0, sBackgroundTemplates, NELEMS(sBackgroundTemplates));
    ScheduleBgCopyTilemapToVram(0);
...
static void Task_ShowResetRtcPrompt(u8 taskId)
{
    ...
    switch (tState)
    {
        case 0:
            ...
            PrintTime(WIN_TIME, 0, 17, gLocalTime.days, gLocalTime.hours, gLocalTime.minutes, gLocalTime.seconds);
            AddTextPrinterParameterized(WIN_TIME, FONT_NORMAL, gText_PreviousTime, 0, 33, TEXT_SKIP_DRAW, 0);
            PrintTime(WIN_TIME, 0, 49, gSaveBlock2Ptr->lastBerryTreeUpdate.days, gSaveBlock2Ptr->lastBerryTreeUpdate.hours, gSaveBlock2Ptr->lastBerryTreeUpdate.minutes, gSaveBlock2Ptr->lastBerryTreeUpdate.seconds);

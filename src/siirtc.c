...
static const char AbgLibRtcVersion[] = "SIIRTC_V001";
...
u8 SiiRtcProbe(void)
{
    ...
    if (!(rtc.status & SIIRTCINFO_24HOUR) || (rtc.status & SIIRTCINFO_POWER)) //modified by red for bugfix
    {
        if (!SiiRtcReset())
        {
            return 0;
